<template>
  <div style="padding: 0">
    <div>
      <router-link :to="`depositHistory`" class="btn btn-primary"
        >history</router-link
      >
    </div>
    <h1 style="font-weight: 800; font-size: larger">deposit orders</h1>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="user in displayorders" :key="user._id">
            ...
            <td>{{ user.userId }}</td>
            <td>
              <router-link :to="`deposit/${user._id}`" class="btn btn-primary"
                >view</router-link
              >
            </td>
          </tr>
...
export default {
  ...
  async created() {
    const response = await AuthenticationServices.getdeposit();
