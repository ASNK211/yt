<template>
  <div style="padding: 0">
    <div>
      <router-link :to="`withdrawhistory`" class="btn btn-primary"
        >history</router-link
      >
    </div>
    <h1 style="font-weight: 800; font-size: larger">withdraw orders</h1>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="user in displayorders" :key="user._id">
            ...
            <td>
              <router-link :to="`withdraw/${user._id}`" class="btn btn-primary"
                >view</router-link
              >
            </td>
          </tr>
